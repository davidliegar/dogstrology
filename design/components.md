...
| Constelación | SVG de `design/constellations/svg`, dos ranuras (`.lines`/`.nodes`), nodo `.dominant` con halo | `colors.constellationLine`, `colors.constellationNode`, `colors.starGlow` | El trazado en el canvas usa un bucle ambiental de 9000ms — **no replicar**: el token real (`motion.duration.trace`, 1200ms) es el revelado único al abrir pantalla, no un bucle. Ver `PLAN.md`, registro 2026-08-25 |
