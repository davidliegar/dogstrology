 ...
 *
 * **Ya no es el destino raíz de la pestaña**: desde el artboard 25 la pestaña
 * abre el hub, y esta pantalla es su tercera fila ("Sus datos"). Por eso se
 * apila, lleva flecha de volver y va por id en la ruta, como los editores que
 * cuelgan de ella.
 *
...
export default function PetProfile() {
  ...
  return (
    <Screen
      ...
      footerDivider
      gap={spacing[5]}
      // Mismo par que la carta: el nombre arriba como cintillo y el título
      // diciendo qué se ve de él. Antes era "Datos de Baloo" porque la
      // pantalla era destino raíz y no tenía nada encima que dijera de quién.
      header={<ScreenHeader divided overline={pet.name()} title="Sus datos" onBack={() => router.back()} />}
      footer={chart ? <ConfidenceMeter confidence={chart.confidence()} /> : null}
    >
      <PetIdentity
        pet={pet}
        photoUri={photoUri ?? undefined}
        onPressAvatar={() => router.push({ pathname: '/pet/[id]/photo', params: { id: pet.id() } })}
      />
      ...
      {chart && notice ? (
        <NoticeCard
          ...
          // La acción lleva al editor del dato que falta. Estaba sin cablear
          // desde F2 —un `onPress` vacío— porque la pantalla era destino raíz
          // y no tenía pila donde apilar el editor; ahora la tiene.
          action={
            notice.action
              ? { label: notice.action, onPress: () => editMissingDatum(chart.confidence(), pet.id()) }
              : undefined
          }
        >
